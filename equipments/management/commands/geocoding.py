...
def ReverseGeocode(lat,lng):
    ...
    try:
        ...
        return [c.full_address,c.city,c.administrative_area]
    except DoesNotExist:
...
def MultispectralGeocode(lat,lng):
    ...
    c = CachedGeocode(
        ...
        administrative_area = title(lower(address[2].text))  
        country = "Brasil"
        postal_code = address[0].get("CEP")
        #TODO: PAREI AQUI!
    )
    c.full_address = c.street+" "+c.number+", "+c.administrative_area+
